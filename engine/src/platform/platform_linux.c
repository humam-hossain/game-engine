...
b8 platform_startup(platform_state* plat_state, const char* application_name, i32 x, i32 y, i32 width, i32 height){
    ...
    u32 even_mask = XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK;
    ...
    if(stream_result <= 0){
        KFATAL("Failed to flush the XCB connection stream: %d", stream_result);
        return FALSE;
    }
    ...
    while(event != 0){
        ...
        switch(event->response_type & ~0x80){
            ...
            case XCB_KEY_RELEASE: {
                // TODO: key press/release event
            } break;
            case XCB_BUTTON_PRESS:
            case XCB_BUTTON_RELEASE: {
                // TODO: mouse button press/release event
            } break;
...
// Sleep on the thread for the provided ms. This blocks the main thread. Should only be used for giving time back to the OS for unused update power. Therefore it is not exported.
